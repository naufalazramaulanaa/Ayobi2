...
export function AppSidebar({
  ...
  const handleSearch = (e: React.FormEvent) => {
    e.preventDefault()
    if (searchQuery.trim()) {
      router.push(`/student/browse-courses?q=${encodeURIComponent(searchQuery.trim())}`)
      setSearchQuery("")
      if (isMobile && onClose) {
        onClose()
      }
  ...
  const getMenuItems = () => {
    ...
    const roleSpecificItems = {
      admin: [
        ...
        { title: "Course Management", icon: BookOpen, page: "course-management" },
      ...
      instructor: [
        ...
        { title: "Live Classes", icon: Video, page: "live-classes", comingSoon: true },
      ...
      student: [
        ...
        { title: "CV Maker", icon: FileUser, page: "cv-maker", comingSoon: true },
        { title: "Referral", icon: Share2, page: "referral" },
        { title: "Become Instructor", icon: UserPlus, page: "become-instructor" },
        { title: "Help", icon: HelpCircle, page: "help" },
        { title: "Profile", icon: User, page: "profile" },
      ],
      reviewer: [
    ...
    return (
      <>
        ...
        >
          ...
          <div className="flex items-center justify-between p-4 border-b border-gray-200 bg-white flex-shrink-0">
            <div className="flex items-center space-x-3 min-w-0 flex-1">
              ...
              <div className="min-w-0 flex-1">
                <h2 className="text-lg font-bold text-midnight-blue-800 truncate">EduLMS</h2>
                <p className="text-sm text-midnight-blue-600 capitalize truncate">{userRole}</p>
              </div>
            </div>
          ...
          <div className="p-4 bg-white border-b border-gray-200 flex-shrink-0">
            <h3 className="text-lg font-semibold text-midnight-blue-800 truncate">{getRoleTitle()}</h3>
          ...
          <div className="flex-1 overflow-y-auto p-4 bg-white">
              ...
              {getMenuItems().map((item) => (
                ...
                >
                  <div className="flex items-center space-x-3 min-w-0 flex-1">
                    ...
                    <span className="font-medium text-midnight-blue-800 truncate">{item.title}</span>
                  </div>
                  {item.comingSoon && (
            ...
            {userRole === "student" && (
              <div className="mt-6">
                <h4 className="text-sm font-semibold text-gray-500 uppercase tracking-wide mb-3">Quick Access</h4>
                <Collapsible>
                  <CollapsibleTrigger className="w-full flex items-center justify-between p-3 rounded-lg hover:bg-gray-100">
                    <div className="flex items-center space-x-3 min-w-0 flex-1">
                      ...
                      <span className="font-medium text-midnight-blue-800 truncate">Categories</span>
                    </div>
                    <ChevronDown className="w-4 h-4 text-midnight-blue-600 flex-shrink-0" />
                  </CollapsibleTrigger>
                  <CollapsibleContent className="mt-2 ml-8 space-y-1">
                    {["Web Development", "Mobile Development", "UI/UX Design", "Data Science"].map((category) => (
                      <button
                        key={category}
              ...
              <DropdownMenuTrigger asChild>
                <button className="w-full flex items-center space-x-3 p-3 rounded-lg hover:bg-gray-100">
                  ...
                  <div className="flex-1 text-left min-w-0">
                    <p className="text-sm font-medium text-midnight-blue-800 truncate">{userEmail}</p>
                    <p className="text-xs text-midnight-blue-600 capitalize truncate">{userRole}</p>
                  </div>
                  <ChevronDown className="w-4 h-4 text-midnight-blue-600 flex-shrink-0" />
                </button>
              </DropdownMenuTrigger>
                ...
                <div className="flex items-center justify-between w-full">
                  <div className="flex items-center space-x-2">
                    ...
                    <div className="min-w-0">
                      <h2 className="text-lg font-bold text-midnight-blue-800 truncate">EduLMS</h2>
                      <p className="text-xs text-midnight-blue-600 capitalize">{userRole}</p>
                    </div>
                  </div>
                  {/* Cart Button for Students */}
                  {userRole === "student" && (
            ...
            <DropdownMenuTrigger asChild>
              ...
              >
                <Avatar className="h-8 w-8 rounded-lg">
                  <AvatarImage src={getUserData().avatar || "/placeholder.svg"} alt={getUserData().name} />
                ...
                <div className="grid flex-1 text-left text-sm leading-tight">
                  <span className="truncate font-semibold">{getUserData().name}</span>
                  <span className="truncate text-xs">{getUserData().email}</span>
                </div>
